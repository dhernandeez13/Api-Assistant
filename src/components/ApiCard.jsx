...
function ApiCard({ api }) {
    ...
    return (
        <div className="relative rounded-lg overflow-hidden bg-gray-900 shadow-lg flex flex-col h-full transition-transform duration-200 hover:-translate-y-1">
            <div className="relative flex items-center justify-center h-48 bg-gray-100">
                ...
                <button
                    className={`favorite-btn absolute top-3 right-3 text-2xl z-10 transition-colors duration-200 ${favorite ? "text-red-500" : "text-gray-400"}`}
            ...
            <div className="p-4 flex-1 flex flex-col gap-2">
                <h3 className="mb-1 text-base font-semibold text-white">
                    <a href={api.html_url} target="_blank" rel="noopener noreferrer" className="hover:underline">{api.full_name}</a>
                </h3>
                <p className="mb-2 text-gray-300 text-sm">{shortDescription}</p>
                <div className="flex flex-wrap gap-4 mb-2 text-sm">
                    <span title="Estrellas" className="flex items-center gap-1 text-yellow-400">
                        ⭐ {api.stargazers_count}
                    </span>
                    <span title="Forks" className="flex items-center gap-1 text-green-400">
                        🍴 {api.forks_count}
                    </span>
                    <span title="Issues abiertas" className="flex items-center gap-1 text-pink-400">
                        🐞 {api.open_issues_count}
                    </span>
                    <span title="Lenguaje principal" className="flex items-center gap-1 text-blue-400">
                        📝 {api.language || "Desconocido"}
                    </span>
                    <span title="Propietario" className="flex items-center gap-1 text-gray-400">
                        👤 {api.owner?.login}
                    </span>
                </div>
            </div>
